...
def get_info(message):
    ...
    text = (
        text
        + "/add: Add a transaction in the format /add @user1 @user2 [Amount] [Memo(optional)]\n\n"
    )
    text = (
        text
        + "/bal: Check a pair of users' balance in the format /bal @user1 @user2\n\n"
    )
    text = (
        text
        + "/rec: Check the past n days transaction record in the format /rec @user1 @user2 [n]\n\n"
    )
    text = (
        text
        + "/delete: Delete a pair of users' transaction record in the format /delete @user1 @user2"
    )
